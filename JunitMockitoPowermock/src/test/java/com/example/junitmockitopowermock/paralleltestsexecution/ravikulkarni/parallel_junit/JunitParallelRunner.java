...
public class JunitParallelRunner {
  ...
  public static class TestClassLoader extends URLClassLoader {
    public TestClassLoader() {
    	...
    	//addURL(new URL( "path/to/other/url/to/search/for/class")); //si apunta a .jar, lo abrirá para buscar la clase!
    }

    @Override
    public Class<?> loadClass(String name) throws ClassNotFoundException {
      if(name.startsWith("com.example.junitmockitopowermock")) {// com.classes.to.load.in.separate.spaces
  ...
  private static class TestRunCallable implements Callable <Result>{
    ...
    public Result call() throws Exception {
      final String oldThreadName = Thread.currentThread().getName();//esto es un thread del pool
      Result result = new Result();
      try {
        Thread.currentThread().setName(testPlanName);//le cambia el nombre temporalmente, para que apunte a un fichero con un nombre legible al hacer console.print... si el threadName no es main, apuntará a otro PrintStream creado bajo demanda y guardado en un HashMap
        ...
        Thread.currentThread().setName(oldThreadName);//devuelve su nombre original
      }
      return result;
    }
  ...
  private static ArrayList<String> getTestPlans() {
    String fileName = "src/test/java/com/example/junitmockitopowermock/paralleltestsexecution/ravikulkarni/parallel_junit/listOfTests.txt";//"listOfTests.txt";
  ...
  public static void main(String args[]) {
    ...
    while (remainingResults > 0) {
      try {
        //Wait for max 1 hour
        completedFuture = completionService.poll(MAX_WAIT_TIMES, TimeUnit.MINUTES);//si uno termina a los MAX-1, vuelve a esperar MAX tiempo.. debería ir reduciéndose el tiempo para considerarse la ejecución de todas las tareas.
        if(completedFuture == null) {
          ...
          runCount = runCount + result.getRunCount();//nbr of methods executed in test classes executed, in this case, one class at the time
          remainingResults--;
    ...
    JunitParallelRunnerPrintStream.flushAndCloseAll();
