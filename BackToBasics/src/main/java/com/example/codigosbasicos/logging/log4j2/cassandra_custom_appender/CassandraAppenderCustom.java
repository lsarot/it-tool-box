 ...
 * @see ColumnMapping
 */
@Plugin(name = "CassandraCustom", category = Core.CATEGORY_NAME, elementType = CassandraAppenderCustom.ELEMENT_TYPE, printObject = true)
public class CassandraAppenderCustom extends AbstractDatabaseAppender<CassandraManagerCustom> {

    private CassandraAppenderCustom(final String name, final Filter filter, final boolean ignoreExceptions,
                              final Property[] properties, final CassandraManagerCustom manager) {
    ...
    public static class Builder<B extends Builder<B>> extends AbstractAppender.Builder<B>
        implements org.apache.logging.log4j.core.util.Builder<CassandraAppenderCustom> {
        ...
        @Override
        public CassandraAppenderCustom build() {
            final CassandraManagerCustom manager = CassandraManagerCustom.getManager(getName(), contactPoints, columns, useTls,
                clusterName, keyspace, table, username, password, useClockForTimestampGenerator, bufferSize, batched,
                batchType);
            return new CassandraAppenderCustom(getName(), getFilter(), isIgnoreExceptions(), null, manager);
        }
