...
@Slf4j //NO SIRVE!, tuve que crear log con un bean aparte.
@Controller
public class RSocketController {
     ...
     * NOTAR QUE
     * usamos PreAuthorize,
     * inyectamos un UserDetails,
     * devolvemos lo mismo de la versión sin security, pero wrapped en Mono o Flux. ( @EnableReactiveMethodSecurity demands that the return values are from project Reactor (i.e. Flux or Mono) )
     */
    @PreAuthorize("hasRole('USER')")
    @MessageMapping("request-response") //declares that any messages with metadata containing the RSocket route of request-response should be handled by this method.  Diferente a HTTP REST RequestMapping, GetMapping, PostMapping, ...
    Mono<Message> requestResponse(final Message request, @AuthenticationPrincipal UserDetails user) {
        ...
        return Mono.just(new Message(SERVER, RESPONSE));
	    		...
	    		/** VERSIÓN SENCILLA (sin security)
	    		 * 
	    		 * USANDO CLI CLIENT (en carpeta spring-rsocket-demo/rsocket-server)
